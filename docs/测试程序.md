...
- **被测站点**：Playwright **`baseURL`**、global setup 与 **`absUrl`** 使用 **`common/global-setup.ts`** 的 **`E2E_BASE_URL`**：**优先**进程环境变量 **`E2E_BASE_URL`**（由 CI **`schedule_*.yml`** 的 **`env`** 写死传入；trim、去尾 `/`），未设置时回退 **`E2E_BASE_URL_DEFAULT`**（默认 **`https://beta.reaslab.io`**）。**Cloudflare / WAF**：默认 **`E2E_CF_TESTING_AUTH`**；若某 origin 需不同头，可在对应 workflow 的 **`env`** 增加 **`E2E_CF_TESTING_AUTH`**。**飞书**「被测网站」见 **`send-feishu.mjs`**。全量定时 **`schedule_beta.yml`**；子集定时 **`schedule_test.yml`** + **`common/run-scope-test.txt`**。
- **本机**：在仓库根 **`reaslab-test`** 下 **`pnpm install`** / **`pnpm run …`**。与 CI 对齐的栈为 **Node 20**、**pnpm 9**、**Playwright Chromium**（具体以 **`package.json`** 与 **`.github/workflows/schedule_beta.yml`** 为准）。**Linux 且 uid=0** 时为 Chromium 附加 **`--no-sandbox`**，避免白屏或登录页元素不出现（**`common/playwright.config.ts`**、**`global-setup.ts`**）。
...
- **定时**：workflow **`.github/workflows/schedule_beta.yml`**（站点 A、全量 **`common/run-scope-beta.txt`**）与 **`.github/workflows/schedule_test.yml`**（站点 B、**`common/run-scope-test.txt`**）均使用 **`on.schedule`**，cron 均可为 **`0 */3 * * *`（UTC）**；二者 **`concurrency.group`** 不同，可并行。换算本地时间见各文件顶部注释。
...
定时 workflow（**`schedule_beta.yml`** / **`schedule_test.yml`**）在跑完 E2E 后会执行 `send-vercel.mjs`，依赖环境变量 **`VERCEL_TOKEN`** 调用 Vercel CLI；**勿**把 token 写进仓库代码。
...
## GitHub Actions 定时任务（`schedule_beta.yml` / `schedule_test.yml`）

对应文件：[`.github/workflows/schedule_beta.yml`](../.github/workflows/schedule_beta.yml)（站点 A、全量）、[`.github/workflows/schedule_test.yml`](../.github/workflows/schedule_test.yml)（站点 B、部分 scope）。

### 触发与并发

- **定时**：`on.schedule`，cron **`0 */3 * * *`**（**UTC**，每 3 小时整点：0、3、6… 时的 `:00`）。换算本地时间请看 workflow 文件顶部注释。
- **手动**：`workflow_dispatch`，可在 Actions 页 **Run workflow**。
- **并发**：**`schedule_beta.yml`** 使用 **`concurrency.group: report-site-a-full`**，**`schedule_test.yml`** 使用 **`report-site-b`**；二者均为 `cancel-in-progress: true`——同一 workflow 内新一次触发会取消仍在跑的上一实例；若不希望被取消，可改为 `false` 或再调稀 cron。
