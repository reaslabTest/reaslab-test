...
- **定时**：workflow **`.github/workflows/schedule_beta.yml`**（全量 **`common/run-scope-beta.txt`**）与 **`.github/workflows/schedule_test.yml`**（子集 **`common/run-scope-test.txt`**）均使用 **`on.schedule`**，cron 均可为 **`0 */3 * * *`（UTC）**；二者 **`concurrency.group`** 不同，可并行。换算本地时间见各文件顶部注释。
