...
- **定时**：workflow **`.github/workflows/schedule_beta.yml`**（全量 **`common/run-scope-beta.txt`**）与 **`.github/workflows/schedule_test.yml`**（子集 **`common/run-scope-test.txt`**）均使用 **`on.schedule`**，cron 均为 **`0 */6 * * *`（UTC）**；二者 **`concurrency.group`** 不同，可并行。换算本地时间见各文件顶部注释。
...
对应文件：[`.github/workflows/schedule_beta.yml`](../.github/workflows/schedule_beta.yml)（全量）、[`.github/workflows/schedule_test.yml`](../.github/workflows/schedule_test.yml)（子集 scope）。
...
- **并发**：**`schedule_beta.yml`** 使用 **`concurrency.group: report-e2e-full-scope`**，**`schedule_test.yml`** 使用 **`report-e2e-partial-scope`**；二者均为 `cancel-in-progress: true`——同一 workflow 内新一次触发会取消仍在跑的上一实例；若不希望被取消，可改为 `false` 或再调稀 cron。
