...
- **被测站点**：Playwright **`baseURL`**、global setup 与 **`absUrl`** 使用 **`common/global-setup.ts`** 的 **`E2E_BASE_URL`**：**优先**进程环境变量 **`E2E_BASE_URL`**（由 CI **`schedule_beta.yml`** 的 **`env`** 写死传入；trim、去尾 `/`），未设置时回退 **`E2E_BASE_URL_DEFAULT`**（默认 **`https://beta.reaslab.io`**）。**Cloudflare / WAF**：默认 **`E2E_CF_TESTING_AUTH`**；若某 origin 需不同头，可在对应 workflow 的 **`env`** 增加 **`E2E_CF_TESTING_AUTH`**。**飞书**「被测网站」见 **`send-feishu.mjs`**。定时全量 E2E 见 **`schedule_beta.yml`**；本地子集可用 **`common/run-scope-test.txt`**。
- **本机**：在仓库根 **`reaslab-test`** 下 **`pnpm install`** / **`pnpm run …`**。与 CI 对齐的栈为 **Node 20**、**pnpm 9**、**Playwright Chromium**（具体以 **`package.json`** 与 **`.github/workflows/schedule_beta.yml`** 为准）。**Linux** 下 Chromium 附加 **`--disable-ipv6`**（WSL 等仅有 AAAA、IPv6 不通时，否则 `global-setup` 的 `page.goto` 易 **`net::ERR_TIMED_OUT`**）；**uid=0** 时再附加 **`--no-sandbox`**，避免白屏或登录页元素不出现（**`common/playwright.config.ts`**、**`global-setup.ts`**）。
...
- **定时**：workflow **`.github/workflows/schedule_beta.yml`**（全量 **`common/run-scope-beta.txt`**）使用 **`on.schedule`**，cron **`0 */6 * * *`（UTC）**。换算本地时间见 workflow 顶部注释。
- **手动**：同一 workflow 支持 **`workflow_dispatch`**，在仓库 **Actions** 页 **Run workflow** 触发，频次不限。
- **本地 / 其它流水线**：无仓库级固定调度；按需执行 **`pnpm run reaslab-test`**（默认按 **`common/run-scope-beta.txt`** 子集）、**`test:NN`**（单文件）等，见 **`README.md`**。
...
2. **`test/`**：主线用例（与 **`docs/用户场景.md`** 章节对应；与 **`reaslab-iipe`** 对照的全量功能与覆盖缺口见 **`docs/用户场景_全量.md`**）共 **16** 个文件：**`01-playground.test.ts`**、**`02-signup.test.ts`**、**`03-loginOut.test.ts`**、**`04-account-settings.test.ts`**、**`05-blankModeling.test.ts`**、**`06-import-git.test.ts`**、**`07-optimizationTemplateModeling.test.ts`**、**`08-theoremTemplateMil.test.ts`**、**`09-modelingContestTemplate.test.ts`**、**`10-feedback.test.ts`**、**`11-footer-links.test.ts`**、**`12-latex.test.ts`**、**`13-project-list.test.ts`**、**`14-browse-templates.test.ts`**、**`15-reaslingo-home.test.ts`**、**`16-reasflow-copilot.test.ts`**；另有共享步骤 **`helpers.ts`**、静态夹具 **`test/data/test_upload.png`**、**`test/data/test_upload.tex`** 等。
...
5. **`run.mjs`** + **`package.json`**：完整链路主入口 **`pnpm run reaslab-test`** 默认 **`--scope-file=common/run-scope-beta.txt`**。失败仍尝试上传报告并通知飞书；默认 **退出码** 反映测试是否通过。GitHub **定时** job 使用 **`pnpm run reaslab-test -- --exit-zero-on-e2e-failure`**（与 **`reaslab-test`** 相同 scope，追加 **`--exit-zero-on-e2e-failure`**）：跑哪些文件、**执行顺序**由 **`common/run-scope-beta.txt`**（**`parse-run-scope.mjs`**）决定；**调度层始终以 0 退出**（Actions 不标红）；**功能层**各 Playwright 用例可有绿/红，结果写在 **`playwright-report`** / **`test-results/`**，Vercel 成功时飞书带报告链接，失败用例亦在报告内可见。
...
flowchart TB
  ...
  subgraph testdir["各test程序"]
    direction TB
    SCENES["主线用例（16 个）<br/>01-playground.test.ts<br/>02-signup.test.ts<br/>03-loginOut.test.ts<br/>04-account-settings.test.ts<br/>05-blankModeling.test.ts<br/>06-import-git.test.ts<br/>07- optimizationTemplateModeling.test.ts<br/>08-theoremTemplateMil.test.ts<br/>09- modelingContestTemplate.test.ts<br/>10-feedback.test.ts<br/>11-footer-links.test.ts<br/>12-latex.test.ts<br/>13-project-list.test.ts<br/>14-browse-templates.test.ts<br/>15-reaslingo-home.test.ts<br/>16-reasflow-copilot.test.ts"]
  end
...
**说明**：图为示意，未画 **`test/data/`**（含 **`.e2e-artifacts/`**）、**`node_modules/`**、**`docs/`**、**`.github/workflows/`** 等。**`docs/用户场景.md`** 为用例文案与编号依据；**`CFG --> GS`** 仅为竖排。**`run.mjs`** 在无 scope 子集时经 **`pnpm exec playwright test --config common/playwright.config.ts`** 跑 **`testMatch`** 全量，再 **`send-results/`**；**`playwright-report/`**、**`test-results/`** 与浏览器断言由 Playwright 写盘，图中不展开。**SCENES** 列 **16** 条主线（**`helpers.ts`** 未入框）。顺序由 **`testMatch`** 与命令行决定；增删或重命名主线文件时请同步改 **SCENES**。**`init`** 中 **`useMaxWidth: false`**，**SCENES** 配 **`min-width`**、**`text-align:left`**；**`07-…`**、**`09-…`** 行在连字符后插入 **`\u00A0`**（图中为不换行空格字符），减轻窄布局下的不当断行。
...
定时 workflow（**`schedule_beta.yml`**）在跑完 E2E 后会执行 `send-vercel.mjs`，依赖环境变量 **`VERCEL_TOKEN`** 调用 Vercel CLI；**勿**把 token 写进仓库代码。
...
## GitHub Actions 定时任务（`schedule_beta.yml`）

对应文件：[`.github/workflows/schedule_beta.yml`](../.github/workflows/schedule_beta.yml)（全量 scope）。

### 触发与并发

- **定时**：`on.schedule`，cron **`0 */6 * * *`**（**UTC**，每 6 小时整点：0、6、12、18 时的 `:00`）。换算本地时间请看 workflow 文件顶部注释。
- **手动**：`workflow_dispatch`，可在 Actions 页 **Run workflow**。
- **并发**：**`concurrency.group: report-beta`**，`cancel-in-progress: true`——同一 workflow 内新一次触发会取消仍在跑的上一实例；若不希望被取消，可改为 `false` 或再调稀 cron。
...
7. **Run scoped E2E (common/run-scope-beta.txt), deploy Vercel, notify Feishu**：**`pnpm run reaslab-test -- --exit-zero-on-e2e-failure`**（**`run.mjs`** 按 **`common/run-scope-beta.txt`**（执行顺序见 **`parse-run-scope.mjs`**）解析 → 仅跑对应 **`test/NN-*.test.ts`** → Vercel 部署 `playwright-report` → 飞书；见上 **`--exit-zero-on-e2e-failure`**）；`env` 注入 **`VERCEL_TOKEN: ${{ secrets.VERCEL_TOKEN }}`**。改跑哪些文件：编辑 **`common/run-scope-beta.txt`**，或在该 step 的 `env` 设 **`E2E_SCOPE_FILE`** 指向其它列表文件。
